...
//	char ttl[4];
	...
	/*
	   ...
	   strcat(r, RTSP_EL);
	 */
	w_pos += snprintf(r + w_pos, sizeof(r) - w_pos, "Session: %d" RTSP_EL, session->session_id);
...
//Temporary disable of multicast code for netembro
	...
	/*
	   ...
	   strcat(r, temp);
	 */
	w_pos += snprintf(r + w_pos, sizeof(r) - w_pos, ";ssrc=%u", rtp_s->ssrc);
	// strcat(r, "\r\n\r\n");
	strcat(r, RTSP_EL RTSP_EL);

	bwrite(r, (unsigned short) strlen(r), rtsp);

	fnc_log(FNC_LOG_CLIENT, "200 - %s ", r_selected_track(rtp_s->track_selector)->info->name);
