...
int RTSP_setup(RTSP_buffer * rtsp, RTSP_session ** new_session)
{
	...
	/* Validate the URL */
	if (!parse_url(url, server, &port, object)) {	//object �il nome del file richiesto
		send_reply(400, 0, rtsp);	/* bad request */
		return ERR_NOERROR;
	}
	...
	if (!valid_url) {	//se l'estensione non �valida
		send_reply(415, 0, rtsp);	/* Unsupported media type */
		return ERR_NOERROR;
	}
	q = strchr(object, '!');
	if (q == NULL) {	//se non c'�"!" non �stato specificato il file che si vuole in streaming (mp3,mpg...)
