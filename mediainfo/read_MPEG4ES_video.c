...
#endif
#if ENABLE_DEBUG
#define DBG_MSG(msg) fprintf(stderr,msg)
#else
#define DBG_MSG(msg)
#endif
	...
	*data_size=0;
	...
	if(!(me->flags & ME_FD)){
		if ( (ret=mediaopen(me)) < 0 ){
			DBG_MSG("Uppss, Error im mediaopen - read_MPEG4ES_video -\n");
			FREE_DATA;
			return ret;
		}
		...
		*recallme=0;
		...
		if(s->final_byte!=VOS_END_CODE && !s->fragmented){
                	data[*data_size]=0x00;
                	*data_size+=1;
                	data[*data_size]=0x00;
                	*data_size+=1;
	...
	while(s->final_byte != VOP_START_CODE){
		if( s->final_byte == VOS_START_CODE){
			/*DBG_MSG("VOS_START_CODE\n");*/
			ret=parse_visual_object_sequence(s,data,data_size,me->fd);
			/*fprintf(stderr,"profile-id=%d\n",s->profile_id);*/
			if(ret!=ERR_NOERROR){
				DBG_MSG("Uppss, Error in parse_visual_object_sequence  - read_MPEG4ES_video -\n");
				FREE_DATA;
				return ret;
			}
		        if(next_start_code(data,data_size,me->fd) < 0){
			...
			/*DBG_MSG("VO_START_CODE\n");*/
			ret=parse_visual_object(data,data_size,me->fd);
			if(ret!=ERR_NOERROR){
				DBG_MSG("Uppss, Error in parse_visual_object - read_MPEG4ES_video -\n");
				FREE_DATA;
				return ret;
			}
			...
			/*DBG_MSG("VIDEO_OBJECT_START_CODE\n");*/
			ret=parse_video_object(data,data_size,me->fd);
			if(ret!=ERR_NOERROR){
				DBG_MSG("Uppss, Error in parse_video_object - read_MPEG4ES_video -\n");
				FREE_DATA;
				return ret;
			}
			s->final_byte=data[*data_size - 1];
			...
			/*DBG_MSG("VIDEO_OBJECT_LAYER_START_CODE\n");*/
			ret=parse_video_object_layer(s,data,data_size,me->fd);
			if(ret!=ERR_NOERROR){
				DBG_MSG("Uppss, Error in parse_video_object_layer - read_MPEG4ES_video -\n");
				FREE_DATA;
				return ret;
			}
			s->final_byte=data[*data_size - 1];
	...
	if(s->final_byte == VOP_START_CODE){
		/*DBG_MSG("VOP_START_CODE\n");*/
		ret=parse_video_object_plane(s,data,data_size,me->fd);
			if(ret!=ERR_NOERROR){
				DBG_MSG("Uppss, Error in parse_video_object_plane - read_MPEG4ES_video -\n");
				FREE_DATA;
				return ret;
			}
			s->final_byte=data[*data_size - 1];
