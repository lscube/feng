...
int read_MPEG4ES_video(media_entry * me, uint8 * data_slot, uint32 * data_size,
		       double *mtime, int *recallme, uint8 * marker)
{				/* reads MPEG4 Elementary stream Video */
	int ret;
	uint32 num_bytes;
	...
	*recallme = 0;
	...
	if (!(me->flags & ME_FD)) {
		if ((ret = mediaopen(me)) < 0)
			return ret;
		s = (static_MPEG4_video_es *) calloc(1,
						     sizeof
	...
	if (s->ref2->vop_time_increment_resolution == 0) {
		/*init */
		s->ref1->vop_time_increment_resolution =
		    s->ref2->vop_time_increment_resolution =
		    me->description.frame_rate;
	}
#if 0
	/*clubbing visual object sequence - visual object - video object - video object layer */
	if ((me->description).msource == live && !s->fragmented
	    && (int) (*mtime) == 0) {
	...
	if (s->fragmented) {
		if (s->remained_data_size > num_bytes) {
			memcpy(data_slot, s->more_data + s->data_read,
			       num_bytes);
	...
	while (s->final_byte != VOP_START_CODE) {
		if (s->final_byte == VOS_START_CODE) {
			ret =
			    parse_visual_object_sequence(s, data, data_size,
							 me->fd);
			if (ret != ERR_NOERROR) {
				FREE_DATA;
				return ret;
			...
			*data_size += 1;
			...
			ret =
			    parse_video_object_layer(s, data, data_size,
						     me->fd);
			if (ret != ERR_NOERROR) {
				FREE_DATA;
				return ret;
	...
	if (s->time_resolution != 0
	    && !(me->description.flags & MED_FORCE_FRAME_RATE))
