...
int read_MPEG_system(media_entry * me, uint8 * data, uint32 * data_size,
		     double *mtime, int *recallme, uint8 * marker)
{
	int ret;
	uint32 num_bytes;
	int count, count1, flag, packet_done = 0, not_remove = 0, time_set =
	    0, pts_diff, clock, dts_present = 0, audio_flag = 0;
	float pkt_len;
	...
	if (!(me->flags & ME_FD)) {
		if ((ret = mediaopen(me)) < 0)
			return ret;
		s = (static_MPEG_system *) calloc(1,
						  sizeof(static_MPEG_system));
		me->stat = (void *) s;
		s->final_byte = 0x00;
		s->offset_flag = 0;
		s->offset = 0;
	...
	*data_size += 1;
	...
	if (s->final_byte == 0xba) {
		read_pack_head(data, data_size, me->fd, &s->final_byte,
			       &s->scr);
	}
	...
	if (s->final_byte >= 0xbc) {
		...
		read_packet_head(data, data_size, me->fd, &s->final_byte,
				 &time_set, &s->pts, &s->dts, &dts_present,
				 &s->pts_audio);
		if (s->new_dts != 1) {
	...
	if (num_bytes != 0) {
		time_set = 0;
		while (((*data_size <= num_bytes) && (*recallme == 1))
		       || (!packet_done)) {
			count =
			    read_packet(data, data_size, me->fd,
					&s->final_byte);
			if (!packet_done) {
				not_remove = 1;
			} else {
		...
		if (!audio_flag) {
			*mtime =
			    ((float) (s->pts.pts - s->offset) * 1000) /
			    (float) clock;
		} else {
			*mtime =
			    ((float) (s->pts_audio.pts - s->offset) * 1000) /
			    (float) clock;
		}
		count = 0;
			...
			*data_size -= count;
			if ((s->pts.pts == s->next_pts.pts)
				...
				*recallme = 1;
				...
				pkt_len =
				    (((float) pts_diff * 1000) / (float) clock);
		...
		*marker = !(*recallme);
		...
		if (s->final_byte == 0xb9) {
			...
			count1 =
			    read_packet_head(data, data_size, me->fd,
					     &s->final_byte, &time_set,
					     &s->next_pts, &s->next_dts,
					     &dts_present, &s->pts_audio);
			count += count1;
			*data_size -= count;
