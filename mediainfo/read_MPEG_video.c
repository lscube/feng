...
int read_MPEG_video(media_entry * me, uint8 * data, uint32 * data_size,
...
{				/* reads MPEG-1,2 Video */
	...
	if ((me->description).msource != live
		...
		if (!me->description.bitrate) {
			...
			me->description.bitrate =
			    (int) (s->data_total * 8 / (*mtime)) * 1000;
		} else {
			s->fragmented = 0;
			wasSeeking = 1;
			if (!flag) {
				lseek(me->fd, 0, SEEK_SET);
				probe_standard(me, data_tmp, data_size, me->fd,
					       &s->std);
				seq_head_pres = 1;
			}
		}
