	     ...
	     int *recallme, uint8 * marker)
{
	...
	*recallme = 0;
	...
	samples =
	    (unsigned int) (me->description.pkt_len *
			    ((float) me->description.clock_rate) / 1000);
	// samples is the number of samples to be reproduced each pkt_len time slot
	start_sample =
	    (unsigned int) (*mtime * me->description.clock_rate / 1000);
	// Current sample to be reproduced

	*buf_size =
	    samples * me->description.bit_per_sample / 8 *
	    me->description.audio_channels;
	...
	for (; me->data_chunk < start_sample; me->data_chunk += samples) {
		if ((me->data_chunk + samples) < start_sample) {	// Some chunks have been lost, so skip them
			lseek(me->fd,
			      samples * me->description.bit_per_sample / 8 *
			      me->description.audio_channels, SEEK_CUR);
		}
	}
	*mtime =
	    (double) me->data_chunk / (double) (me->description.clock_rate) *
	    1000;

	for (i = 0; i < *buf_size; i += 2) {
		if (read(me->fd, &(buffer[i + 1]), 1) <= 0)
			break;
		if (read(me->fd, &(buffer[i]), 1) <= 0)
			break;
	}
	*buf_size = i;
