...
int read_seq_head(media_entry * me, uint8 * buf, uint32 * buf_size, int fin,
...
int read_gop_head(uint8 * buf, uint32 * buf_size, int fin,
		  unsigned char *final_byte, char *hours, char *minutes,
		  char *seconds, char *picture, standard std)
{				/* reads GOP header */
	int count = 0;
...
int read_picture_head(uint8 * buf, uint32 * buf_size, int fin,
		      unsigned char *final_byte, char *temp_ref,
		      video_spec_head1 * vsh1, standard std)
{				/* reads picture head */
	int count = 0;
...
int read_slice(uint8 * buf, uint32 * buf_size, int fin, char *final_byte)
{				/* reads a slice */
...
int probe_standard(media_entry * me, uint8 * buf, uint32 * buf_size, int fin,
		   standard * std)
...
int read_picture_coding_ext(uint8 * buf, uint32 * buf_size, int fin,
