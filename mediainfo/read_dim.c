...
int read_dim(int file, long int *dim)
{
	...
	if ((n = read(file, &byte1, 1)) != 1) {
		fnc_log(FNC_LOG_ERR,
			"Errore durante la lettura della dimensione del tag ID3  al byte1\n");
		return ERR_PARSE;
	}
	res = calculate_skip((int) byte1, &skip, 24);
	if ((n = read(file, &byte2, 1)) != 1) {
		fnc_log(FNC_LOG_ERR,
			"Errore durante la lettura della dimensione del tag ID3 al byte2\n");
		return ERR_PARSE;
	}
	res = calculate_skip((int) byte2, &skip, 16);
	if ((n = read(file, &byte3, 1)) != 1) {
		fnc_log(FNC_LOG_ERR,
			"Errore durante la lettura della dimensione del tag ID3 al byte3\n");
		return ERR_PARSE;
	}
	...
	if ((n = read(file, &byte4, 1)) != 1) {
		fnc_log(FNC_LOG_ERR,
			"Errore durante la lettura della dimensione del tag ID3 al byte4\n");
		return ERR_PARSE;
	}
