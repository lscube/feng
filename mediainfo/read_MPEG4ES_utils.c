...
int parse_visual_object_sequence(static_MPEG4_video_es * mpeg4_struct,
				 uint8 * data, uint32 * data_size, int fin)
{
...
int parse_visual_object(uint8 * data, uint32 * data_size, int fin)
{
	...
	*data_size += 1;
	...
	if (get_field(data, 1, &off))
		off += 7;	/*if is_visual_oject_identifier => skip visual_object_verid (4 bits) and visual_object_priority (3 bits) */
			     ...
			     uint32 * data_size, int fin)
{
	...
	*data_size += 1;
	...
	if (out->ref2 == NULL) {
		out->ref2 =
		    (mpeg4_time_ref *) calloc(1, sizeof(mpeg4_time_ref));
	}
	if (out->ref1 == NULL) {
		out->ref1 =
		    (mpeg4_time_ref *) calloc(1, sizeof(mpeg4_time_ref));
	...
	out->ref1->vop_time_increment_resolution =
	    out->ref2->vop_time_increment_resolution;
	i = out->ref2->vop_time_increment_resolution =
	    get_field(data, 16, &off);

	if (!get_field(data, 1, &off)) {
		/*missing marker */
		return ERR_PARSE;
	}
	for (out->vtir_bitlen = 0; i != 0; i >>= 1)
		++out->vtir_bitlen;
...
//      while(  get_field( data, 1, &off )!=0 ) modulo_time_base++;
	...
	if (out->ref2 == NULL)
		out->ref2 =
		    (mpeg4_time_ref *) calloc(1, sizeof(mpeg4_time_ref));
	out->ref1->var_time_increment = out->ref2->var_time_increment =
	    get_field(data, out->vtir_bitlen, &off);
	if (out->time_resolution == 0 && out->ref2->var_time_increment != 0)
		out->time_resolution =
		    ((double) out->ref2->var_time_increment /
		     (double) out->ref2->vop_time_increment_resolution) * 1000;
	...
	if (out->vop_coding_type != 2) {	/* not B-FRAME */
		...
		out->timestamp =
		    ((double) out->ref2->modulo_time_base *
		     out->ref2->vop_time_increment_resolution +
		     (double) out->ref2->var_time_increment) * (1000 /
								(double) out->
								ref2->
		...
		if (out->timestamp < out->last_non_b_timestamp) {
			...
			out->timestamp =
			    out->last_non_b_timestamp + out->time_resolution;
		}
		if (out->timestamp >
		    (out->last_non_b_timestamp + 6 * out->time_resolution))
			out->timestamp =
			    out->last_non_b_timestamp + out->time_resolution;
		...
		out->timestamp =
		    ((double) out->ref1->modulo_time_base *
		     out->ref1->vop_time_increment_resolution +
		     (double) out->ref1->var_time_increment) * (1000 /
								(double) out->
								ref1->
								vop_time_increment_resolution);
