			...
			if (strcasecmp(keyword, SD_ENCODING_NAME) == 0) {
				...
				if ((strstr
				     (p->description.encoding_name,
				      "H26L") != 0)
				    ||
				    (strstr(p->description.encoding_name, "MPV")
				     != 0)
				    ||
				    (strstr
				     (p->description.encoding_name,
