...
static int mpv_packetize(uint8_t *dst, uint32_t *dst_nbytes, uint8_t *src, uint32_t src_nbytes, MediaProperties *properties, void *private_data)
{
    ...
    mpv->is_fragmented=1; /*assume fragmented by default*/
...
/*        if(mpv->is_fragmented) {
    ...
    VSHCPY;
    fnc_log(FNC_LOG_ERR, "[MPV] Frag: count=%d, dst_nbytes=%d, mpv->countsrc=%d, src_nbytes=%d, S=%d B=%d E=%d",
            count, *dst_nbytes, mpv->countsrc, src_nbytes, mpv->vsh1->s, mpv->vsh1->b, mpv->vsh1->e);
    *dst_nbytes=count;

    mpv->vsh1->s=0;/*RESET. sequence header is not present. SEQ_HEADER can be present only in the first pkt of a frame */
    if (!mpv->is_fragmented)
        fnc_log(FNC_LOG_ERR, "[MPV] End of Frags");
          ...
          long extradata_len)
{
    ...
    int ret;
//    BPSlot *slot;
	uint32_t dst_len = DEFAULT_MTU; //len + 4; //mpegaudio can be fragmented but not collated
    uint8_t dst[DEFAULT_MTU + 4];
    do {
...
{
    mpv_data *mpeg_video = private_data;
    if (private_data) {
